...
mkdir ../kidiff
find . -name "*.kicad_pcb" -not -path "./DELTA-Pro_lib/*" -exec bash -c 'gendiffs "$0"' {} \;
if [ "$DELTA-Pro_SSH_PASS" != "" ] && [ "$DELTA-Pro_SSH_USER" != "" ] && [ "$DELTA-Pro_SSH_SERVER" != "" ]; then
  tar -czf - -C ../kidiff . | sshpass -p "$DELTA-Pro_SSH_PASS" ssh -o StrictHostKeyChecking=no "$DELTA-Pro_SSH_USER"@"$DELTA-Pro_SSH_SERVER" "tar -xzf - -C docs/diffs/"
fi
