...
mkdir ../kidiff
find . -name "*.kicad_pcb" -not -path "./gerefi_lib/*" -exec bash -c 'gendiffs "$0"' {} \;
if [ "$GEREFI_SSH_PASS" != "" ] && [ "$GEREFI_SSH_USER" != "" ] && [ "$GEREFI_SSH_SERVER" != "" ]; then
  tar -czf - -C ../kidiff . | sshpass -p "$GEREFI_SSH_PASS" ssh -o StrictHostKeyChecking=no "$GEREFI_SSH_USER"@"$GEREFI_SSH_SERVER" "tar -xzf - -C docs/diffs/"
fi
